...
from apps.backend.core.schemas import ComprehensionResult, Plan, TaskNode, TraceLog
        ...
        if comprehension.domain_classified == "videojuegos":
            tasks = [
                TaskNode(
                    ...
                    agent_role="Designer",
                    description="Diseñar mecánicas y dinámicas del juego",
                ),
                TaskNode(
                    task_id=f"{plan_id}_task_2",
                    agent_role="Programmer",
                    description="Escribir lógica y bucle principal de juego",
                    dependencies=[f"{plan_id}_task_1"],
                ),
                TaskNode(
                    task_id=f"{plan_id}_task_3",
                    agent_role="Verifier",
                    description="Validar código y mecánicas",
            ...
            tasks = [
                TaskNode(
                    ...
                    description="Diseñar arquitectura de componentes y base de datos",
                ),
                TaskNode(
                    task_id=f"{plan_id}_task_2",
                    agent_role="Programmer",
                    description="Implementar controladores y rutas API",
                    dependencies=[f"{plan_id}_task_1"],
                ),
                TaskNode(
                    task_id=f"{plan_id}_task_3",
                    agent_role="Verifier",
                    description="Verificar cobertura de pruebas y calidad",
                    dependencies=[f"{plan_id}_task_2"],
                ),
            ]
        else:
            tasks = [
                TaskNode(
                    ...
                    description="Investigar y recopilar información clave",
                ),
                TaskNode(
                    task_id=f"{plan_id}_task_2",
                    agent_role="Writer",
                    description="Redactar borrador del documento",
                    dependencies=[f"{plan_id}_task_1"],
                ),
                TaskNode(
                    task_id=f"{plan_id}_task_3",
                    agent_role="Verifier",
                    description="Verificar precisión y coherencia",
                    dependencies=[f"{plan_id}_task_2"],
                ),
            ]
