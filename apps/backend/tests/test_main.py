...
sys.path.append(".")
from apps.backend.brain.database import Base, ProjectModel, TaskModel, get_db
...
async def test_process_request_videojuegos_and_database_persistence():
    ...
    async with TestingSessionLocal() as session:
        ...
        proj_query = await session.execute(
            select(ProjectModel).filter_by(session_id="session_db_123")
        )
        ...
        tasks_query = await session.execute(
            select(TaskModel).filter_by(project_id=proj.id)
        )
