...
class BaseLLMProvider(ABC):
    ...
    async def generate_response(
        self, prompt: OrchestratedPrompt
    ) -> Tuple[str, int, float]:
        """
...
class OpenAIProvider(BaseLLMProvider):
    ...
    async def generate_response(
        ...
        logger.info(
            f"OpenAIProvider: Generating response for model {prompt.model_name}"
        )
        ...
        if (
            settings.openai_api_key == "mock-openai-api-key"
            or not settings.openai_api_key
        ):
            logger.info(
                "OpenAIProvider: Using simulated SDK response (mock key active)"
            )
...
class AnthropicProvider(BaseLLMProvider):
    ...
    async def generate_response(
        ...
        logger.info(
            f"AnthropicProvider: Generating response for model {prompt.model_name}"
        )
        ...
        # Self-contained implementation with API keys fallback check:
        if (
            settings.anthropic_api_key == "mock-anthropic-api-key"
            or not settings.anthropic_api_key
        ):
            logger.info(
                "AnthropicProvider: Using simulated SDK response (mock key active)"
            )
            ...
            logger.info(
                "AnthropicProvider: Call completed through official API channel"
            )
