...
class QdrantVectorStore(BaseVectorStore):
    ...
    async def connect(self) -> None:
        ...
        """
        logger.info(
            f"QdrantVectorStore: Connecting to client at {self._host}:{self._port}"
        )
    ...
    async def search_similarity(
        ...
        if not self._client:
            logger.warning(
                "QdrantVectorStore: Client not connected. Returning simulated similarity memory matches."
            )
            return [
                {
                    "id": "mem_1",
                    "text": f"Simulated memory matching query: {query}",
                    "score": 0.95,
                },
                {
                    "id": "mem_2",
                    "text": "Preferencias de desarrollo web minimalistas",
                    "score": 0.88,
                },
            ]

        logger.info(
            f"QdrantVectorStore: Performing semantic similarity search for '{query}' with limit {limit}"
        )
        try:
            ...
            return [
                {
                    "id": "mem_1",
                    "text": f"Simulated live memory matching query: {query}",
                    "score": 0.95,
                }
            ]
    ...
    async def upsert_vector(
        self, document_id: str, text: str, vector: List[float], metadata: Dict[str, Any]
    ) -> None:
        """
        ...
        if not self._client:
            logger.warning(
                "QdrantVectorStore: Client not initialized. Simulating vector registration."
            )
            return
        ...
        try:
            # self._client.upsert(...)
            logger.info(
                f"QdrantVectorStore: Registered document vector '{document_id}' into Qdrant collection"
            )
        except Exception as e:
            logger.error(
                f"QdrantVectorStore: Failed to register vector inside Qdrant: {str(e)}"
            )
