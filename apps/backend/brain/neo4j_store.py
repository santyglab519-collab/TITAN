...
class Neo4jGraphStore(BaseGraphStore):
    ...
    def __init__(
        self, uri: str = "bolt://localhost:7687", auth: Optional[tuple] = None
    ):
        self._uri = uri
    ...
    async def get_neighbors(self, node_id: str, depth: int = 1) -> List[Dict[str, Any]]:
        ...
        if not self._driver:
            logger.warning(
                "Neo4jGraphStore: Driver not connected. Returning simulated mock data."
            )
            return [
                {
                    "source": node_id,
                    "relation": "RELATED_TO",
                    "target": "SimulatedConcept",
                    "weight": 1.0,
                }
            ]

        logger.info(
            f"Neo4jGraphStore: Fetching neighbors for node {node_id} at depth {depth}"
        )
        query = (
    ...
    async def add_relation(
        self, source_id: str, relation: str, target_id: str, weight: float = 1.0
    ) -> None:
        """
        ...
        if not self._driver:
            logger.warning(
                "Neo4jGraphStore: Driver not connected. Simulating relation save."
            )
            return
        ...
        try:
            async with self._driver.session() as session:
                ...
                logger.info(
                    f"Neo4jGraphStore: Saved relation {source_id} -[{relation}]-> {target_id} (weight={weight})"
                )
        except Exception as e:
